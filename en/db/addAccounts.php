...
<section>
    <script>
        function redirectToHomepage() {
            ...
            setInterval(function () {
                document.getElementById("textshow").innerHTML = "You will be redirected in " + sec + " seconds";
                sec--;
            }, 1000);
            setTimeout(function () {
                window.location.replace("http://edu.uhk.cz/~hejzlzd1/en/#usersTable-section");
            }, 25000);
        }
    </script>
    <?php
    ...
    if ($conn->connect_error) {
        ...
        if (isset($_REQUEST['username']) && isset($_REQUEST['password']) && isset($_REQUEST['customMessage'])) {
            $testCount = "select * from safe";
            $result = $conn->query($testCount);
            if (mysqli_num_rows($result) > 49) {
                print('
...
<div class="alert alert-danger d-flex align-items-center" role="alert">
  ...
  <div>
    The maximum limit of records in the database has been reached - delete records.<br/>
    <div id="textshow">You will be redirected in 25 seconds</div>
      <script>
           redirectToHomepage();
    </script>
    </div>
                ...
                $encryptedpass = base64_encode(openssl_encrypt(bin2hex($pass), 'blowfish', bin2hex("dbkey"), OPENSSL_RAW_DATA, base64_decode("LHG0xiZoQHuWqOWR+CdJPg==")));
...
<div class="alert alert-danger d-flex align-items-center" role="alert">
  <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg>
  <div>
    There was an error in adding a user. One of the parameters was not entered.
